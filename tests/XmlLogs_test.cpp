...
private slots:
    ...
    void parseXml_data()
    {
        ...
        std::transform(shortTextLevels_s.cbegin(), shortTextLevels_s.cend(), std::back_inserter(shortTextLevels), [](const QString& line) {
            return MessageLevel::fromName(line.trimmed());
        });
    ...
    void parseXml()
    {
        ...
        std::transform(entries.cbegin(), entries.cend(), std::back_inserter(levels), [](std::pair<MessageLevel, QString> entry) {
            return entry.first;
        });
