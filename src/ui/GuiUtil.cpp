    ...
    if (dialog.execWithTask(job.get()) == QDialog::Accepted) {
        if (pasteJob->pasteLink().isEmpty()) {
            CustomMessageBox::selectable(
                parentWidget, QObject::tr("Failed to upload logs!"), "The upload link is empty", QMessageBox::Critical)
                ->show();
            return {};
        }
...
static QStringList
BrowseForFileInternal(QString context, QString caption, QString filter, QString defaultPath, QWidget* parentWidget, bool single)
{
