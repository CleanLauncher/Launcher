...
bool createInstanceShortcut(const Shortcut& shortcut, const QString& filePath)
{
    ...
    if (appPath.startsWith("/private/var/")) {
        QMessageBox::critical(shortcut.parent,
                              QObject::tr("Create Shortcut"),
...
#endif
        QMessageBox::critical(
            shortcut.parent, QObject::tr("Create Shortcut"), QObject::tr("Failed to create %1 shortcut!").arg(shortcut.targetString));
        return false;
    }
...
bool createInstanceShortcutOnDesktop(const Shortcut& shortcut)
{
    ...
    if (!createInstanceShortcut(shortcut, shortcutFilePath))
        return false;
    QMessageBox::information(shortcut.parent,
                             QObject::tr("Create Shortcut"),
...
bool createInstanceShortcutInApplications(const Shortcut& shortcut)
{
    ...
    if (!applicationsDirQ.mkpath(".")) {
        QMessageBox::critical(
            shortcut.parent, QObject::tr("Create Shortcut"), QObject::tr("Failed to create instances folder in applications folder!"));
        return false;
    }
#endif
...
bool createInstanceShortcutInOther(const Shortcut& shortcut)
{
    ...
    shortcutFilePath = fileDialog.getSaveFileName(
        shortcut.parent, QObject::tr("Create Shortcut"), shortcutFilePath, QObject::tr("Desktop Entries") + " (*" + extension + ")");
    ...
    QMessageBox::information(
        shortcut.parent, QObject::tr("Create Shortcut"), QObject::tr("Created a shortcut to this %1!").arg(shortcut.targetString));
    return true;
}
