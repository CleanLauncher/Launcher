...
#include "updater/updater/UpdaterDialogs.h"
...
void appDebugOutput(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    UpdaterApp* app = static_cast<UpdaterApp*>(QCoreApplication::instance());
...
UpdaterApp::UpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    QString executable = FS::PathCombine(binPath, BuildConfig.LAUNCHER_APP_BINARY_NAME);
    ...
    auto update_url = parser.value("update-url");
    if (update_url.isEmpty())
        update_url = BuildConfig.UPDATER_GITHUB_REPO;
    ...
    if (marker_file.exists()) {
        ...
        QMetaObject::invokeMethod(this, &UpdaterApp::loadReleaseList, Qt::QueuedConnection);
    }
}
...
void UpdaterApp::fail(const QString& reason)
{
...
void UpdaterApp::abort(const QString& reason)
{
    qCritical() << qPrintable(reason);
    m_status = Aborted;
    exit(2);
...
void UpdaterApp::run()
{
    ...
    qDebug() << "loading exe at" << m_executable;
    ...
    qDebug() << "Executable reports as:" << m_binaryName << "version:" << m_version;
    qDebug() << "Version major:" << m_versionMajor;
    qDebug() << "Version minor:" << m_versionMinor;
    qDebug() << "Version minor:" << m_versionPatch;
    qDebug() << "Version channel:" << m_versionChannel;
    qDebug() << "Git Commit:" << m_gitCommit;
...
void UpdaterApp::moveAndFinishUpdate(QDir target)
{
...
QList<GitHubRelease> UpdaterApp::nonDraftReleases()
{
...
QList<GitHubRelease> UpdaterApp::newerReleases()
{
    QList<GitHubRelease> newer;
    for (auto rls : nonDraftReleases()) {
        if (rls.version > m_version)
            newer.append(rls);
    }
    return newer;
}

GitHubRelease UpdaterApp::selectRelease()
{
...
QList<GitHubReleaseAsset> UpdaterApp::validReleaseArtifacts(const GitHubRelease& release)
{
...
void UpdaterApp::performUpdate(const GitHubRelease& release)
{
...
void UpdaterApp::clearUpdateLog()
{
    FS::deletePath(m_updateLogPath);
}

void UpdaterApp::logUpdate(const QString& msg)
{
...
void UpdaterApp::performInstall(QFileInfo file)
{
    ...
    if (m_isPortable || file.fileName().endsWith(".zip") || file.fileName().endsWith(".tar.gz")) {
        write_lock_file(update_lock_path, QDateTime::currentDateTime(), m_version, m_install_release.tag_name, m_rootPath, m_dataPath);
...
void UpdaterApp::unpackAndInstall(QFileInfo archive)
{
...
bool UpdaterApp::loadVersionFromExe(const QString& exe_path)
{
    QProcess proc = QProcess();
    ...
    if (!version_parts.isEmpty())
        ...
        m_versionPatch = 0;
    m_gitCommit = lines.takeFirst().simplified();
    return true;
}

void UpdaterApp::loadReleaseList()
{
    auto github_repo = m_repoUrl;
...
GitHubRelease UpdaterApp::getLatestRelease()
{
...
bool UpdaterApp::needUpdate(const GitHubRelease& release)
{
    auto current_ver = Version(QString("%1.%2.%3").arg(m_versionMajor).arg(m_versionMinor).arg(m_versionPatch));
    return current_ver < release.version;
}

void UpdaterApp::downloadError(QString reason)
{
